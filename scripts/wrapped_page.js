...
const chartMargin = { top: 30, right: 30, bottom: 60, left: 100 };
...
const forceGraphSliderValueSpan = document.getElementById(
  "forceGraphSliderValue"
);
...
(async function loadData() {
  ...
  try {
    ...
    if (allParsedData.length === 0) {
      ...
      if (mainChartDiv)
        mainChartDiv.innerHTML = `<p class="error-message">No valid data found.</p>`;
    ...
    if (startDateInput && endDateInput && overallMinDate && overallMaxDate) {
      ...
      console.log(
        `Set initial date range inputs: ${minDateStr} to ${maxDateStr}`
      );
    } else {
    ...
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="error-message">Error loading data.</p>`;
...
function drawLineGraph(dataToDisplay, displayStartDate, displayEndDate) {
  console.log(
    `drawLineGraph called for range: ${
      displayStartDate ? formatDate(displayStartDate) : "N/A"
    } to ${displayEndDate ? formatDate(displayEndDate) : "N/A"}`
  );
  mainChartDiv.innerHTML = "";
  d3.select(mainChartDiv).selectAll("p.peak-instructions").remove();
  d3.select(mainChartDiv)
    .append("p")
  ...
  svgInstance = null;
  lineGraphXScale = null; // Clear previous
  ...
  const validDisplayStartDate =
    displayStartDate instanceof Date && !isNaN(displayStartDate)
      ? d3.timeDay.floor(displayStartDate)
      : null;
  const validDisplayEndDate =
    displayEndDate instanceof Date && !isNaN(displayEndDate)
      ? d3.timeDay.floor(displayEndDate)
      : null;

  // Handle cases where the range is invalid or no listening data exists
  if (
    !validDisplayStartDate ||
    !validDisplayEndDate ||
    validDisplayStartDate > validDisplayEndDate
  ) {
    console.error(
      "drawLineGraph: Invalid display date range.",
      validDisplayStartDate,
      validDisplayEndDate
    );
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="error-message">Invalid date range for display.</p>`;
  ...
  if (listeningData.length === 0 && dataToDisplay.length > 0) {
    console.log(
      "drawLineGraph: No listening data (ms_played > 0) for the provided data subset."
    );
    ...
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="empty-message">No data for this period.</p>`;
  ...
  const dailyDataMap = d3.rollup(
    ...
    (d) => d3.timeDay.floor(d.ts)
  );

  // Create data points *only* for the days within the specified display range
  const daysInDisplayRange = d3.timeDays(
    validDisplayStartDate,
    d3.timeDay.offset(validDisplayEndDate, 1)
  );
  ...
  const containerWidth =
    mainChartDiv.clientWidth > 0 ? mainChartDiv.clientWidth * 0.95 : 540;
  ...
  if (width <= 0 || height <= 0) {
    ...
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="error-message">Container too small.</p>`;
    return;
  }
  ...
  const xScale = d3
    .scaleTime()
    .domain([validDisplayStartDate, validDisplayEndDate])
    .range([0, width]);
  ...
  chartGroup
    ...
    .attr(
      "transform",
      `translate(${width / 2}, ${height + chartMargin.bottom - 5})`
    )
  ...
  const lineGen = d3
    ...
    .defined(
      (d) =>
        d.value > 0 &&
        d.date >= validDisplayStartDate &&
        d.date <= validDisplayEndDate
    )
    .curve(d3.curveMonotoneX);
  ...
  const mainContentGroup = chartGroup
    .append("g")
  ...
  chartGroup
    .append("rect")
    .attr("class", "tooltip-overlay") // Use specific class
    .attr("width", width)
    .attr("height", height)
    .style("fill", "none")
  ...
  function mousemove(event) {
    ...
    const d0 = processedDisplayData[i - 1];
    const d1 = processedDisplayData[i];
    ...
    if (d) {
      ...
      focus
        ...
        .attr(
          "transform",
          `translate(${lineGraphXScale(d.date)}, ${yScale(d.value)})`
        );
      showTooltip(
        event,
        `${formatDate(d.date)}<br><b>Listened: ${formatTime(d.value)}</b>`
      );
    } else {
      focus.style("display", "none");
      hideTooltip();
  ...
  async function brushed({ selection, sourceEvent }) {
    ...
    if (!selection) {
      ...
      const [viewStartDate, viewEndDate] = d3.extent(
        currentViewData,
        (d) => d.ts
      );
      ...
      if (
        ...
        viewStartDate &&
        viewEndDate &&
        (currentDomain[0].getTime() !== viewStartDate.getTime() ||
          currentDomain[1].getTime() !== viewEndDate.getTime())
      ) {
        console.log(
          "Resetting zoom to full view range:",
          viewStartDate,
          viewEndDate
        );
        ...
        console.log(
          "Brush cleared, but already displaying full view range. No redraw needed."
        );
      }
      return; // Exit handler
    }
    ...
    if (!lineGraphXScale) {
      console.error("brushed: lineGraphXScale is null!");
      return;
    }
    ...
    if (
      ...
      isNaN(flooredDate0) ||
      isNaN(flooredDate1) ||
      flooredDate0.getTime() >= flooredDate1.getTime()
    ) {
      console.warn(
        "Invalid date range from brush (start >= end or NaN). Clearing brush."
      );
    ...
    console.log(
      `Brush selected range: ${formatDate(flooredDate0)} to ${formatDate(
        flooredDate1
      )}`
    );
    ...
    const filteredForDependents = currentViewData.filter(
      (d) => d.ts >= filterStart && d.ts < filterEnd
    );
    console.log(
      `Filtered data for dependent charts count: ${filteredForDependents.length}`
    );
  ...
  const brushGroup = chartGroup
    ...
    .call(lineGraphBrush);
  console.log(
    "Brush applied AFTER tooltip overlay. Brush Group Element:",
    brushGroup.node()
  );
...
async function updateTopArtistsAsText(data) {
  ...
  const imageFetchPromises = artistData.map(
    async ([artist, totalMinutes], index) => {
      ...
      const trackWithUri = artistTracks
        .sort((a, b) => b.ms_played - a.ms_played)[0];
  ...
  try {
    ...
    results.forEach((result) => {
      ...
      li.innerHTML = `
        ...
        result.artist
      }" class="artist-img" style="margin-right: var(--spacing);" />
...
async function updateTopTracksAsText(data) {
  ...
  const imageFetchPromises = trackData.map(
    async ([trackArtist, totalMinutes], index) => {
    ...
    results.forEach((result) => {
      ...
      li.innerHTML = `
        ...
        <span class="track-info" style="flex-grow: 1;">
          <span class="track-name" style="display: block; font-weight: bold;">${
            result.index + 1
          }. ${truncateText(result.trackName, 25)}</span>
          <span class="track-artist" style="display: block; font-size: 0.9em; color: #555;">${truncateText(
            result.artistName,
            30
          )}</span>
        </span>
        <span class="track-time" style="margin-left: auto; padding-left: 10px; white-space: nowrap;"> (${formatTime(
          result.totalMinutes
        )})</span>
      `;
      if (result.trackName.length > 25)
        li.querySelector(".track-name").title = result.trackName;
      if (result.artistName.length > 30)
        li.querySelector(".track-artist").title = result.artistName;
...
function updateTimeOfDayChart(data) {
  ...
  svg
    ...
    .enter()
    .append("rect")
    .attr("class", "bar time-of-day-bar")
    .attr("x", (d) => x(d[0]))
...
function updateDayOfWeekChartAsText(data) {
  ...
  targetDiv.innerHTML = "";
  if (!data || data.length === 0) {
    targetDiv.innerHTML = `<p class="empty-message" style="color: var(--dark-green-color);">No data.</p>`;
    return;
  }
  ...
  const peakDayIndex = sortedDays.length > 0 ? sortedDays[0][0] : -1;
  const peakMinutes = sortedDays.length > 0 ? sortedDays[0][1] : 0;
  let textContent = `<h4 style="color: var(--spotify-color); margin-bottom: 5px;">Listening by Day</h4>`;
  textContent += `<p style="margin-top: 0; margin-bottom: 5px;">Total: <strong>${formatTime(
    totalMinutes
  )}</strong></p>`;
  if (peakDayIndex !== -1) {
    textContent += `<p style="margin-top: 0; margin-bottom: 10px;">Most active: <strong style="color: var(--dark-green-color);">${
      dayOfWeekNames[peakDayIndex]
    }</strong> (${formatTime(peakMinutes)}).</p>`;
  }
  textContent += `<ol style="padding-left: 1.2rem; margin: 0; font-size: 0.9em; line-height: 1.4;">`;
  const dayOrder = [1, 2, 3, 4, 5, 6, 0];
  /* Mon-Sun */ for (const dayIndex of dayOrder) {
    const minutes = dayMap.get(dayIndex) || 0;
    textContent += `<li style="margin-bottom: 2px;"><strong>${
...
async function drawForceGraph2(
  filteredData,
  containerId,
  topN = currentForceGraphTopN
) {
  ...
  for (let i = 0; i < musicData.length - 1; i++) {
    ...
    if (
      topArtistsSet.has(sourceArtist) &&
      topArtistsSet.has(targetArtist) &&
  ...
  const nodes = Array.from(topArtistsSet).map((artist) => ({
    id: artist,
    playCount: artistCounts.get(artist) || 0,
  }));
  ...
  function highlight(event, d_hovered) {
    ...
    link.style("stroke", (l) =>
      l.source === d_hovered || l.target === d_hovered ? "#555" : "#999"
    );
  ...
  if (descEl && descEl.classList.contains("chart-description")) {
    descEl.innerHTML = `Transitions between top ${nodes.length} artists (max ${topN} shown). Hover/Pan/Zoom.`;
  }
}

async function handleBrushUpdate(filteredChartData) {
  console.log(
    `handleBrushUpdate called with ${filteredChartData.length} records.`
  );
...
async function updateVisualization(filteredData) {
  ...
  if (!currentViewData || currentViewData.length === 0) {
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="empty-message">No data found for the selected period.</p>`;
    chartsToClear.forEach((el) => {
      if (el) el.innerHTML = `<p class="empty-message">No data.</p>`;
    });
  ...
  if (
    ...
    if (mainChartDiv)
      mainChartDiv.innerHTML = `<p class="error-message">Invalid date range in data.</p>`;
    updateFilterInfoLabel(null, null);
    return;
  }

  console.log(
    `Rendering view for: ${formatDate(viewStartDate)} to ${formatDate(
      viewEndDate
    )}`
  );
...
async function applyDateRangeFilter() {
  ...
  console.log(
    `Auto-updating view for range ${formatDate(start)} to ${formatDate(end)}`
  );
...
function setupEventListeners() {
  ...
  if (wrappedYearSelect) {
    wrappedYearSelect.onchange = async () => {
      ...
      if (selectedValue === "all") {
        ...
        console.log("Updating view for 'All Time'");
        dataToRender = allParsedData;
        ...
        if (isNaN(selectedYear)) {
          ...
          const effectiveStartDate =
            !overallMinDate || yearStart < overallMinDate
              ? overallMinDate
              : yearStart;
          const effectiveEndDate =
            !overallMaxDate || yearEnd > overallMaxDate
              ? overallMaxDate
              : yearEnd;
          const effectiveEndFilter = d3.timeDay.offset(effectiveEndDate, 1);
          dataToRender = allParsedData.filter(
            (d) => d.ts >= effectiveStartDate && d.ts < effectiveEndFilter
          );
          if (startDateInput)
            startDateInput.value = formatDateForInput(effectiveStartDate);
          if (endDateInput)
            endDateInput.value = formatDateForInput(effectiveEndDate);
          console.log(`Updating view for year ${selectedYear}`);
        }
      } else {
  ...
  if (applyRangeBtn) {
    applyRangeBtn.onclick = async () => {
      ...
      start =
        !overallMinDate || start < overallMinDate ? overallMinDate : start;
      end = !overallMaxDate || end > overallMaxDate ? overallMaxDate : end;
      startDateInput.value = formatDateForInput(start);
      endDateInput.value = formatDateForInput(end);
      const filterEnd = d3.timeDay.offset(end, 1);
      ...
      console.log(
        `Updating view for range ${formatDate(start)} to ${formatDate(end)}`
      );
      await updateVisualization(filteredByRange);
    };
  } else {
  ...
  if (forceGraphSlider && forceGraphSliderValueSpan) {
    ...
    forceGraphSlider.addEventListener("change", async () => {
      ...
      if (currentDomain && currentDomain.length === 2) {
        ...
        dataForForceGraph = currentViewData.filter(
          (d) => d.ts >= filterStart && d.ts < filterEnd
        );
        console.log(
          `Updating Force Graph based on current graph zoom (${dataForForceGraph.length} records)`
        );
      } else {
        dataForForceGraph = currentViewData;
        console.log(
          `Updating Force Graph based on main view data (${dataForForceGraph.length} records)`
        );
      }
      const fgContainer = document.getElementById("force-graph-chart");
      if (dataForForceGraph && dataForForceGraph.length > 0) {
        await drawForceGraph2(
          dataForForceGraph,
          "force-graph-chart",
          currentForceGraphTopN
        );
      } else {
        console.log("Slider changed, but no data for force graph.");
        if (fgContainer)
          fgContainer.innerHTML =
            '<p class="empty-message">Select data to see transitions.</p>';
      }
    });
  } else {
    console.warn("Force graph slider elements not found.");
  ...
  if (resetRangeBtn) {
    resetRangeBtn.onclick = async () => {
      console.log("Reset to All Time clicked");
      if (!overallMinDate || !overallMaxDate) {
        console.error("Cannot reset: overall date range is missing.");
        return;
      }

      // Update inputs and dropdown
      if (startDateInput && endDateInput) {
        startDateInput.value = formatDateForInput(overallMinDate);
        endDateInput.value = formatDateForInput(overallMaxDate);
      }
      if (wrappedYearSelect) {
        wrappedYearSelect.value = "all";
      }

      // Reset to full data
      await updateVisualization(allParsedData);
    };
  } else {
    console.warn("resetRangeBtn not found.");
  }
